...
void	*philo_routine(void *pointer) // funkcja przyjmuje argument "pointer", ktory jest wskaznikiem typu "void *". Jest to standardowa sygnatura funkcji uzywanej do tworzenia watkow w C.
{
	...
	if(philo->id % 2 == 0) // jezeli "id" filozofa jest parzyste...
		ft_usleep(1); // (tu chyba mozna po prostu napisac usleep(1000) zamiast tworzyc nowa funkcje)... to usypiamy go na 1 milisekunde; robimy to aby unikac ryzyka zakleszczen, czyli nachodzenia na siebie watkow. Dzieki temu unikamy ryzyka, ze dwoch filozofow siegnie po te same widelce w tym samym czasie
	while(!dead_loop(philo)) // dopoki flozof nie jest martwy (nie zakonczyl dzialania)
	{
		eat(philo);
		dream(philo);
		think(philo);
...
int	thread_create(t_program *program, pthread_mutex_t *forks)
{
	...
	pthread_create((&observer, NULL, &monitor, program->philos) != 0) // pthread_create tworzy watek obserwatora, ktory bedzie wykonywal funkcje "monitor" i przekaze ja jako argument "program->philos". Jesli utworzy go z sukcesem to zwraca 0. W kazdym innym przypadku jest blad.
